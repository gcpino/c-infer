/* test_conv2d.c -- Conv2D parity test. */
#include "cinfer.h"
#include "test_harness.h"
#include <stdlib.h>

int main(void) {
    CINF_RUN(test_conv2d);
    size_t bsz, esz, isz;
    unsigned char* buf = cinf_read_file("build/test_fixtures/conv2d.cinf", &bsz);
    unsigned char* exp = cinf_read_file("build/test_fixtures/conv2d.expected.bin", &esz);
    unsigned char* inb = cinf_read_file("build/test_fixtures/conv2d.input.bin", &isz);
