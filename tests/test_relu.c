...
int main(void) {
    CINF_RUN(test_relu);
    size_t bsz, esz, isz;
    unsigned char* buf = cinf_read_file("build/test_fixtures/relu.cinf", &bsz);
    unsigned char* exp = cinf_read_file("build/test_fixtures/relu.expected.bin", &esz);
    unsigned char* inb = cinf_read_file("build/test_fixtures/relu.input.bin", &isz);
